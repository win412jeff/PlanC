// 取得 #cygintroduction 元素的位置及高度
const intro = document.getElementById('cygintroduction');
const introPos = intro.offsetTop;
const introHeight = intro.offsetHeight;

// 取得 #app 和 #imgGroup 元素
...
window.addEventListener('scroll', () => {
  ...
  // 判斷是否進入 #cygintroduction 區塊
  if (scrollPos >= introPos && scrollPos < introPos + introHeight) {
    // 進入區塊，設定 #app 和 #imgGroup 元素的位置為 fixed
      app.style.position = 'fixed';
      imgGroup.style.position = 'fixed';
      gsap.set("#scrollDist", {
    ...
    // 離開區塊，設定 #app 和 #imgGroup 元素的位置為其他適當值
    app.style.position = 'static';
    imgGroup.style.position = 'static';
  }
});
